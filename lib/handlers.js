...
handlers.index = (requestData, currentProtocol) => {
    ...
    const templateData = {
        'head.title': 'Home',
        'head.description': 'Pizza-delivery company',
        'body.class': 'index'
    };
    ...
    return Promise.all([helpers.getTemplate('_header'), 
        ...
        .then(([header, body, footer]) => {
            ...
            if (!page) {
                return { statusCode: 500, body: '<h1>Oops, something went wrong. Try again later</h1>', contentType: 'html' };
            }
            return { statusCode: 200, body: page, contentType: 'html' }
        })
        .catch(e => {
            if (e.message == 'Internal server error') {
                return { statusCode: 500, body: '<h1>Oops, something went wrong. Try later again</h1>', contentType: 'html' };
            }
            if (e.message == 'Not found') {
                return { statusCode: 404, body: '<h1>This page was not found</h1>', contentType: 'html' };
            }
        });
};
